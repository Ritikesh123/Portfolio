...
export default function Portfolio() {
  return (
    <section id="work" name="work" className="mesh-light my-6 py-24 md:my-8 md:py-32">
      <div className="section-shell">
        <div className="mb-12 flex items-end justify-between gap-6">
          ...
          <p className="hidden max-w-xs text-sm leading-6 text-[#aeb0b9] md:block">Personal projects shaped by curiosity, responsive UI and a strong product instinct.</p>
        </div>
        <div className="grid gap-x-7 gap-y-14 md:grid-cols-2">
          {projects.map(([number, title, detail, image, url, code]) => (
            <article key={number} className="group">
              <a href={url} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-2xl border border-white/10 bg-[#1b1e28]"><img src={image} alt={title} className="aspect-[1.45] w-full object-cover transition duration-500 group-hover:scale-105" /></a>
              <div className="mt-4 flex items-start justify-between gap-4"><div><p className="eyebrow">{number}</p><h3 className="mt-2 text-xl font-extrabold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-[#aeb0b9]">{detail}</p></div><div className="flex gap-3 pt-1 text-white"><a aria-label={`${title} live demo`} href={url} target="_blank" rel="noreferrer"><FiArrowUpRight /></a><a aria-label={`${title} source code`} href={code} target="_blank" rel="noreferrer" className="text-xs font-bold">Code</a></div></div>
            </article>
          ))}
        </div>
      </div>
